...
const isRegisteredUser = () => {
    // Only treat as registered when the session is active.
    // This keeps the register popup available after logout.
...
async function handleCTA() {
    ...
    try {
        const res = await fetch(apiUrl(`/api/check-user-status?email=${encodeURIComponent(email)}`));
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            const normalizedClasses = String(this.className || '').toLowerCase();
            const isRegisterCTA = this.classList.contains('get-assistance-btn') ||
                                  this.classList.contains('register-cta-button') ||
                                  this.classList.contains('btn-primary-massive') ||
                                  this.classList.contains('promo-cta') ||
                                  normalizedClasses.includes('get-assistance-btn') ||
                                  normalizedClasses.includes('register-cta-button') ||
    ...
    document.addEventListener('click', (e) => {
        ...
        const cta = target.closest('.get-assistance-btn, .register-cta-button, .course-register-btn, .promo-cta, .btn-primary-massive');
        if (!cta) return;
        ...
        const tryInitialize = () => {
            ...
            const initGoogle = () => {
                if (typeof google === 'undefined' || !google.accounts || !google.accounts.id) {
                    ...
                    // After retries, show the fallback button
                    setFallbackVisible(true);
                    return;
                }
                ...
                try {
                    ...
                    // If render produced the native button, keep fallback hidden.
                    setTimeout(() => {
    ...
    // The init function is defined above and will be invoked after the modal is injected.
    ...
    async function handleCredentialResponse(response) {
        ...
        try {
            ...
            try {
                ...
                try {
                    if (email) {
                        ...
                        if (syncResponse.ok && syncData?.success) {
                            if (syncData.userId) {
                                try { sessionStorage.setItem('currentUserId', syncData.userId); } catch {}
                                try { localStorage.setItem('currentUserId', syncData.userId); } catch {}
                            }
                        }
                ...
                // Let the success banner show briefly, then route by completion state.
                ...
                try { await routeSignedInUserToCorrectPage(); } catch (err) { try { window.location.replace('next-form.html'); } catch (e) { window.location.href = 'next-form.html'; } }
            }
        } catch (err) {
            console.error('Google sign-in callback failed:', err);
            try { await routeSignedInUserToCorrectPage(); } catch (e) { try { window.location.href = 'next-form.html'; } catch { /* ignore */ } }
        }
    }
    ...
    (async () => {
        ...
        openRegModal = async () => {
            ...
            if (isActiveSession() && isStep2Pending()) {
                window.location.href = 'next-form.html';
                return;
            }
            ...
            if (modalInjected) {
                // Add the modal-open class first so CSS makes the registration
                // container fixed immediately and prevents any layout reflow
                // when we unhide the injected section. This avoids the footer
                // jumping into place briefly before the overlay becomes fixed.
                document.body.classList.add('reg-modal-open');
            ...
            regOverlay.hidden = false;
            document.body.classList.add('reg-modal-open');
            // Also add the class again on the next frame so opacity/animation transitions trigger reliably.
        ...
        try {
            window.__closeRegModal = closeRegModal;
        ...
        document.addEventListener('click', (e) => {
            const btn = e.target instanceof Element ? e.target.closest('#regModalClose') : null;
            if (btn && typeof closeRegModal === 'function') {
                e.preventDefault();
                closeRegModal();
            }
        });
        ...
        document.addEventListener('click', (e) => {
            ...
            const normalizedClasses = String(btn.className || '').toLowerCase();
            const isRegisterCTA = btn.classList.contains('get-assistance-btn') ||
            ...
            const isExplicitRegistrationLink = href === '#registration-section' ||
                                                href === 'index.html#registration-section' ||
                                                href.endsWith('#registration-section');

            const shouldOpenRegistration = isRegisterCTA ||
                                           (isHomePagePath() && isExplicitRegistrationLink);

            if (!shouldOpenRegistration) return;

            if (isActiveSession() && isRegisteredUser()) {
                e.preventDefault();
                e.stopPropagation();
...
let authFormListenersBound = false;
...
const getAuthElements = () => ({
    ...
    authSocialBlock: document.getElementById('authSocialBlock'),
    forgotPasswordLink: document.getElementById('forgotPasswordLink'),
    forgotPasswordForm: document.getElementById('forgotPasswordForm'),
    forgotSendCodeBtn: document.getElementById('forgotSendCodeBtn'),
    forgotBackToLogin: document.getElementById('forgotBackToLogin')
});

// Auth form binder is defined later once the DOM has registration markup available.
...
function setAuthMode(mode) {
    ...
    if (authSocialBlock) authSocialBlock.hidden = !isSignup;
...
let verifyForgotPasswordCode = async () => {};
...
const { forgotPasswordForm, forgotSendCodeBtn } = getAuthElements();
...
if (forgotSendCodeBtn) {
    ...
    forgotSendCodeBtn.addEventListener('click', async () => {
        ...
        try {
            const resp = await fetch(apiUrl('/api/forgot-password'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
if (forgotCodeInput) {
    ...
    const tryVerifyCode = async () => {
        ...
        try {
            const resp = await fetch(apiUrl('/api/verify-reset-code'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    verifyForgotPasswordCode = tryVerifyCode;
}
...
if (forgotPasswordForm) {
    forgotPasswordForm.addEventListener('submit', async (e) => {
        ...
        if (!__resetCodeVerified) {
            ...
            if (!/^[0-9]{6}$/.test(String(code || ''))) return showNotification('Please enter the 6-digit code', 'error');
        ...
        try {
            const resp = await fetch(apiUrl('/api/reset-password'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
const bindAuthForms = () => {
    registrationForm = document.getElementById('registrationForm') || registrationForm;
    loginForm = document.getElementById('loginForm') || loginForm;
    if (!registrationForm && !loginForm) return;
    if (authFormListenersBound) return;
    authFormListenersBound = true;
    ...
    registrationForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // DEBUG HOOK: capture attempted registration payload for E2E testing
            try { window.__lastRegistrationPayload = JSON.parse(JSON.stringify(registrationData)); } catch (e) { /* ignore */ }

            const endpoint = editMode ? apiUrl('/api/update-registration') : apiUrl('/api/register');
            ...
            // DEBUG HOOK: capture server response for E2E testing
            try { window.__lastRegistrationResponse = payload; } catch (e) { /* ignore */ }
            if (response.status === 409 && payload?.alreadyRegistered) {
                showNotification(payload?.error || 'You have already signed in. Please log in instead.', 'info');
            ...
            const submissionId = String(payload?.userId || currentUserId || normalizedEmail).trim();
            ...
            sessionStorage.setItem('currentUserId', submissionId);
            localStorage.setItem('currentUserId', submissionId);
            ...
            // Log form data
            console.log('Registration successful:', { editMode, submissionId, email: normalizedEmail });
            ...
            const regData = { fullName, email: normalizedEmail, phone };
            ...
            console.warn('Registration save failed:', error);

            const errorMsg = String(error?.message || 'Registration failed.').trim();

            // Smart handling for duplicate accounts if the doc already exists.
            if (errorMsg.includes('already registered') || errorMsg.includes('already signed in') || errorMsg.includes('exists')) {
                showNotification('You have already signed in. Switching to Login...', 'info');
...
function showNotification(message, type = 'info') {
    ...
    const autoDismissMs = type === 'error' ? 6500 : 3000;
    setTimeout(() => {
