 ...
 * @param {string} path - The endpoint path (e.g., '/api/register.php-step2')
...
async function handleCTA() {
    ...
    // Not logged in -> open the registration/auth flow on the current page.
    if (!email) {
        try {
            if (typeof showRegistrationSection === 'function') {
                showRegistrationSection({ preferLogin: false });
...
document.addEventListener('click', function(e) {
    const selector = 'a[href="#registration-section"], a[href$="#registration-section"], a.register-scroll, a.register-cta-button, a.btn-primary-massive, a.promo-cta, button.register-scroll, button.register-cta-button, button.get-assistance-btn, button.promo-cta, button.btn-primary-massive';
    let cta = e.target && typeof e.target.closest === 'function' ? e.target.closest(selector) : null;
    if (!cta && e.target && e.target.parentElement && typeof e.target.parentElement.closest === 'function') {
        cta = e.target.parentElement.closest(selector);
    }
    if (!cta) return;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    (async () => {
        ...
        openRegModal = async () => {
            ...
            // Consume the force flag and any pending schedule once opened.
            try {
                sessionStorage.removeItem('av:shouldShowRegistration');
                sessionStorage.removeItem('av:showRegistrationAt');
                sessionStorage.removeItem('forceOpenRegistration');
                localStorage.removeItem('forceOpenRegistration');
            } catch {
                // ignore
            }
        };
        ...
        if (window.location.hash === '#registration-section') {
            ...
            // Do not show the registration section immediately on home page load.
            // Instead, auto-open the modal after the intended delay for logged-out users.
            hideRegistrationSection();

            if (!isActiveSession()) {
                regModalTimer = setTimeout(() => {
                    if (isModalOpen() || isActiveSession() || isRegisteredUser() || shouldRedirectToCongrats()) return;
                    openRegModal();
                }, OPEN_DELAY_MS);
            }
...
// Form Validation and Submission - Wrap in DOMContentLoaded
document.addEventListener('DOMContentLoaded', () => {
...
}); // Close the registration DOMContentLoaded handler
